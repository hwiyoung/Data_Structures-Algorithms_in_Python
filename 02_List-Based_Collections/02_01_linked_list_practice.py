...
class LinkedList(object):
    ...
    def get_position(self, position):
        ...
        if position < 1:
            return None
        # 1. Check whether the current instance has value
        while current and counter <= position:
            # 2. Check whether position is in the list
            if counter == position:
                return current
            current = current.next
            counter += 1
    ...
    def insert(self, new_element, position):
        ...
        if position > 1:
            while current and counter < position:
                # 1. Get position of 'new_element'
                if counter == position - 1:
                    # 2. Switch each next(pointer) of each node
